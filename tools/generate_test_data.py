...
def main(only_exchange=None):
    ...
    print(
        f"Generating test data. This will take approximately {(len(EXCHANGE_MAP) - len(set(skip))) * 0.5} minutes."
    )
    loop = asyncio.get_event_loop()
    for exch_str, exchange in (
        EXCHANGE_MAP.items()
        if only_exchange is None
        else [(only_exchange, EXCHANGE_MAP[only_exchange])]
    ):
        if exch_str in skip:
            continue
        ...
        channels = list(
            set.intersection(
                set(info["channels"]["websocket"]),
                set([L2_BOOK, TRADES, TICKER, CANDLES]),
            )
        )
        sample_size = 10
