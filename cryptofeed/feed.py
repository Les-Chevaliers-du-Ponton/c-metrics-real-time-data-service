...
class Feed(Exchange):
        ...
        **kwargs,
        ...
        if self.valid_candle_intervals != NotImplemented:
            if candle_interval not in self.valid_candle_intervals:
                raise ValueError(
                    f"Candle interval must be one of {self.valid_candle_intervals}"
                )

        if self.candle_interval_map != NotImplemented:
            self.normalize_candle_interval = {
                value: key for key, value in self.candle_interval_map.items()
            }
        ...
        if symbols and channels:
            ...
            channels = list(
                set([self.std_channel_to_exchange(chan) for chan in channels])
            )
    ...
    def connect(
        ...
        Tuple[AsyncConnection, Callable[[None], None], Callable[[str, float], None]]
    ]:
        """
        ...
        for endpoint in self.websocket_endpoints:
            ...
            temp_sub = {
                chan: [self.exchange_symbol_to_std_symbol(s) for s in symbols]
                for chan, symbols in self.subscription.items()
            }
            filtered_sub = {
                chan: [self.std_symbol_to_exchange_symbol(s) for s in symbols]
                for chan, symbols in endpoint.subscription_filter(temp_sub).items()
            }
    ...
    def address(self) -> Union[List, str]:
        if len(self.websocket_endpoints) == 0:
            return
        addrs = [
            ep.get_address(sandbox=self.sandbox) for ep in self.websocket_endpoints
        ]
        ...
        if len(bid) > 0 and len(ask) > 0:
            ...
            if best_bid >= best_ask:
                raise BidAskOverlapping(
                    f"{self.id} - {data.symbol}: best bid {best_bid} >= best ask {best_ask}"
                )
        ...
        for callbacks in self.callbacks.values():
            for callback in callbacks:
                if hasattr(callback, "stop"):
                    LOG.info(
                        "%s: stopping backend %s", self.id, self.backend_name(callback)
                    )
                    await callback.stop()
        ...
        for callbacks in self.callbacks.values():
            for callback in callbacks:
                if hasattr(callback, "start"):
                    LOG.info(
                        "%s: starting backend task %s with multiprocessing=%s",
                        self.id,
                        self.backend_name(callback),
                        "True" if self.config.backend_multiprocessing else "False",
                    )
    ...
    def backend_name(self, callback):
        if hasattr(callback, "__class__"):
            if hasattr(callback, "handler"):
                return (
                    callback.handler.__class__.__name__
                    + "+"
