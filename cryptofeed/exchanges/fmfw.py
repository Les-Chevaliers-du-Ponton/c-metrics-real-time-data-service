...
class FMFW(Feed):
    ...
    rest_endpoints = [
        RestEndpoint("https://api.fmfw.io", routes=Routes("/api/3/public/symbol"))
    ]
        ...
        for sym, updates in msg["update"].items():
            ...
            for u in updates:
                c = Candle(
                    ...
                    u["t"] / 1000,
                    u["t"] / 1000
                    + timedelta_str_to_sec(self.normalize_candle_interval[interval])
                    - 0.1,
                ...
                json.dumps(
                    {
                        ...
                        "ch": (
                            chan
                            if chan != "candles/"
                            else chan + self.candle_interval_map[self.candle_interval]
                        ),
                        "id": 1234,
                    }
                )
