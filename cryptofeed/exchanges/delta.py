...
class Delta(Feed):
    ...
    websocket_endpoints = [
        WebsocketEndpoint(
            "wss://socket.delta.exchange", sandbox="wss://testnet-socket.delta.exchange"
        )
    ]
    rest_endpoints = [
        RestEndpoint("https://api.delta.exchange", routes=Routes("/v2/products"))
    ]
    ...
    def _parse_symbol_data(cls, data: dict) -> Tuple[Dict, Dict]:
        ...
        for entry in data["result"]:
            ...
            if entry["contract_type"] == "spot":
                ...
                entry["contract_type"] == "futures"
                or entry["contract_type"] == "move_options"
            ):
                sym = Symbol(
                    base, quote, type=FUTURES, expiry_date=entry["settlement_time"]
                )
            elif (
                entry["contract_type"] == "call_options"
                or entry["contract_type"] == "put_options"
            ):
    ...
    async def _book(self, msg: dict, timestamp: float):
        ...
        self._l2_book[symbol].book.bids = {
            Decimal(e["limit_price"]): Decimal(e["size"]) for e in msg["buy"]
        }
        self._l2_book[symbol].book.asks = {
            Decimal(e["limit_price"]): Decimal(e["size"]) for e in msg["sell"]
        }
