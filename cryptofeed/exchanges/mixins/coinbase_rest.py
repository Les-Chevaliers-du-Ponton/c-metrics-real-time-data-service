...
class CoinbaseRestMixin(RestExchange):
    ...
    async def _request(
        ...
        if method == "GET":
            ...
            data = await self.http_conn.write(
                f"{api}{endpoint}",
                msg=json.dumps(body),
                header=header,
                retry_count=retry_count,
            ...
            data = await self.http_conn.delete(
                f"{api}{endpoint}",
                header=header,
                retry_count=retry_count,
                retry_delay=retry_delay,
    ...
    async def _date_to_trade(self, symbol: str, timestamp: float) -> int:
        ...
        while True:
            data = await self._request(
                "GET", f"/products/{symbol}/trades?after={bound}"
            )
        ...
        if start:
            ...
            while True:
                ...
                if limit > 0:
                    data = await self._request(
                        "GET",
                        f"/products/{symbol}/trades?after={start_id}&limit={limit}",
                        retry_count=retry_count,
                        retry_delay=retry_delay,
                    )
                    data = list(reversed(data))
    ...
    async def trade_history(self, symbol: str, start=None, end=None):
        data = await self._request(
            "GET", f"/orders?product_id={symbol}&status=done", auth=True
        )
        return [
            {
                "order_id": order["id"],
                ...
                "price": Decimal(order["executed_value"])
    ...
    async def candles(
        self,
        symbol: str,
        start: Optional[Union[str, dt, float]] = None,
        end: Optional[Union[str, dt, float]] = None,
        ...
        ), f'Interval must be one of {", ".join(list(valid_intervals.keys()))}'
        ...
        if start:
            ...
            while True:
                ...
                data = await self._request(
                    "GET", url, retry_count=retry_count, retry_delay=retry_delay
                )
                data = list(reversed(data))
                yield list(
                    map(lambda x: self._candle_normalize(symbol, x, interval), data)
                )
                await asyncio.sleep(1 / self.request_limit)
            ...
            data = await self._request(
                "GET",
                f"/products/{symbol}/candles?granularity={valid_intervals[interval]}",
                retry_count=retry_count,
                retry_delay=retry_delay,
            )
