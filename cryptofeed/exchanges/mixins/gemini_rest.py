...
class GeminiRestMixin(RestExchange):
    ...
    def _order_status(self, data):
        ...
        price = (
            Decimal(data["price"])
            if Decimal(data["avg_execution_price"]) == 0
            else Decimal(data["avg_execution_price"])
        )
        return {
            "order_id": data["order_id"],
            "symbol": self.exchange_symbol_to_std_symbol(
                data["symbol"].upper()
            ),  # Gemini uses lowercase symbols for REST and uppercase for WS
    ...
    async def _get(self, command: str, retry_count, retry_delay, params=""):
        ...
        resp = await self.http_conn.read(
            f"{api}{command}{params}", retry_count=retry_count, retry_delay=retry_delay
        )
    ...
    async def l2_book(self, symbol: str, retry_count=1, retry_delay=60):
        ...
        ret.book.bids = {
            Decimal(u["price"]): Decimal(u["amount"]) for u in data["bids"]
        }
        ret.book.asks = {
            Decimal(u["price"]): Decimal(u["amount"]) for u in data["asks"]
        }
        return ret
    ...
    async def trades(
        ...
        while True:
            data = reversed(
                await self._get(
                    f"/v1/trades/{sym}?", retry_count, retry_delay, params=params
                )
            )
            if end:
    ...
    async def place_order(
        ...
        parameters = {
            ...
            "price": str(price),
            "options": (
                [self.normalize_order_options(o) for o in options] if options else []
            ),
        }
