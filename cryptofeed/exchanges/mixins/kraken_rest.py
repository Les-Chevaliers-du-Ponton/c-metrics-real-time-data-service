...
class KrakenRestMixin(RestExchange):
    ...
    order_options = {LIMIT: "limit", MARKET: "market", MAKER_OR_CANCEL: "post"}
    ...
    async def _post_public(
        self, command: str, payload=None, retry_count=1, retry_delay=60
    ):
        url = f"{self.api}{command}"
        resp = await self.http_conn.write(
            url,
            msg={} if not payload else payload,
    ...
    async def _post_private(self, command: str, payload=None):
        ...
        signature = hmac.new(base64.b64decode(self.key_secret), message, hashlib.sha512)
        sigdigest = base64.b64encode(signature.digest())

        headers = {"API-Key": self.key_id, "API-Sign": sigdigest.decode()}

        resp = await self.http_conn.write(
            f"{self.api}{command}", msg=payload, header=headers
        )
    ...
    async def trades(
        ...
        if start and end:
            async for data in self._historical_trades(
                symbol, start, end, retry_count, retry_delay
            ):
                data = data["result"]
    ...
    async def _historical_trades(
        self, symbol, start_date, end_date, retry_count, retry_delay
    ):
    ...
    async def place_order(
        ...
        if options:
            parameters["oflags"] = ",".join(
                [self.normalize_order_options(self.id, o) for o in options]
            )
    ...
    async def trade_history(self, symbol: str = None, start=None, end=None):
        ...
        for trade_id, trade in data["result"]["trades"].items():
            ...
            ret[trade_id] = {
                ...
                "fee_currency": (
                    symbol.split("-")[1] if symbol else std_sym.split("-")[1]
                ),
