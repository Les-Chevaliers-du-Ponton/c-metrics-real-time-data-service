...
class BinanceRestMixin(RestExchange):
    ...
    def _generate_signature(self, query_string: str):
        h = hmac.new(
            self.key_secret.encode("utf8"), query_string.encode("utf8"), hashlib.sha256
        )
        return h.hexdigest()

    async def _request(
        self, method: str, endpoint: str, auth: bool = False, payload={}, api=None
    ):
    ...
    async def place_order(
        ...
        data = await self._request(
            POST, "test" if test else "order", auth=True, payload=parameters
        )
        return data
    ...
    async def cancel_order(self, order_id: str, symbol: str):
        sym = self.std_symbol_to_exchange_symbol(symbol)
        data = await self._request(
            DELETE, "order", auth=True, payload={"symbol": sym, "orderId": order_id}
        )
        return data
    ...
    async def orders(self, symbol: str = None):
        data = await self._request(
            ...
            auth=True,
            payload=(
                {"symbol": self.std_symbol_to_exchange_symbol(symbol)} if symbol else {}
            ),
        )
        return data
    ...
    async def order_status(self, order_id: str):
        data = await self._request(
            GET, "order", auth=True, payload={"orderId": order_id}
        )
        return data
...
class BinanceFuturesRestMixin(BinanceRestMixin):
    ...
    async def balances(self):
        data = await self._request(
            GET, "account", auth=True, api="https://fapi.binance.com/fapi/v2/"
        )
        return data["assets"]
