...
class Gemini(Feed, GeminiRestMixin):
    id = GEMINI
    websocket_channels = {L2_BOOK: L2_BOOK, TRADES: TRADES, ORDER_INFO: ORDER_INFO}
    websocket_endpoints = [
        WebsocketEndpoint(
    ...
    def __reset(self, pairs):
        for pair in pairs:
            self._l2_book[self.exchange_symbol_to_std_symbol(pair)] = OrderBook(
                self.id,
    ...
    def generate_token(self, payload=None) -> dict:
        ...
        signature = hmac.new(
            self.key_secret.encode("utf-8"), b64_payload, hashlib.sha384
        ).hexdigest()
    ...
    async def message_handler(self, msg: str, conn: AsyncConnection, timestamp: float):
        ...
        if "type" not in msg:
            ...
            or msg["type"] == "auction_indicative"
            or msg["type"] == "auction_open"
        ):
            return
        else:
    ...
    async def _ws_authentication(self, address: str, options: dict) -> Tuple[str, dict]:
        ...
        symbols = "&".join(
            [f"symbolFilter={s.lower()}" for s in symbols]
