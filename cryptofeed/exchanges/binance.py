...
class Binance(Feed, BinanceRestMixin):
    ...
    websocket_endpoints = [
        WebsocketEndpoint(
            "wss://stream.binance.com:9443", sandbox="wss://testnet.binance.vision"
        )
    ]
    rest_endpoints = [
        RestEndpoint(
    ...
    def __init__(self, depth_interval="100ms", **kwargs):
        ...
        """
        if (
            depth_interval is not None
            and depth_interval not in self.valid_depth_intervals
        ):
            raise ValueError(
    ...
    def _address(self) -> Union[str, Dict]:
        ...
        is_any_public = any(
            not self.is_authenticated_channel(chan) for chan in self.subscription
        )
    ...
    def _check_update_id(self, std_pair: str, msg: dict) -> bool:
        ...
        """
        if (
            self._l2_book[std_pair].delta is None
            and msg["u"] <= self.last_update_id[std_pair]
        ):
            return True
        elif (
            msg["U"] <= self.last_update_id[std_pair]
            and msg["u"] <= self.last_update_id[std_pair]
        ):
    ...
    async def _snapshot(self, pair: str) -> None:
        ...
        resp = await self.http_conn.read(
            self.rest_endpoints[0].route("l2book", self.sandbox).format(pair, max_depth)
        )
        ...
        self._l2_book[std_pair] = OrderBook(
            ...
            bids={Decimal(u[0]): Decimal(u[1]) for u in resp["bids"]},
            asks={Decimal(u[0]): Decimal(u[1]) for u in resp["asks"]},
        )
    ...
    async def _funding(self, msg: dict, timestamp: float):
        ...
        f = Funding(
            ...
            self.timestamp_normalize(msg["E"]),
            predicted_rate=(
                Decimal(msg["P"]) if "P" in msg and msg["P"] is not None else None
            ),
            raw=msg,
        )
    ...
    async def _order_update(self, msg: dict, timestamp: float):
        ...
        oi = OrderInfo(
            ...
            (
                LIMIT
                if msg["o"].lower() == "limit"
                else MARKET if msg["o"].lower() == "market" else None
            ),
            (
                Decimal(msg["Z"]) / Decimal(msg["z"])
                if not Decimal.is_zero(Decimal(msg["z"]))
                else None
            ),
            Decimal(msg["q"]),
