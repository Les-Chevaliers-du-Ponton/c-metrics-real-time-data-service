...
class HuobiSwap(HuobiDM):
    ...
    websocket_endpoints = [
        WebsocketEndpoint(
            "wss://api.hbdm.com/swap-ws", instrument_filter=("QUOTE", ("USD",))
        ),
        WebsocketEndpoint(
            "wss://api.hbdm.com/linear-swap-ws", instrument_filter=("QUOTE", ("USDT",))
        ),
    ]
    rest_endpoints = [
        RestEndpoint(
    ...
    async def _funding(self, pairs):
        ...
        while True:
            for pair in pairs:
                ...
                received = time.time()
                update = (
                    data["data"]["funding_rate"],
                    self.timestamp_normalize(int(data["data"]["funding_time"])),
                )
                if (
                    pair in self.funding_updates
                    and self.funding_updates[pair] == update
                ):
                ...
                f = Funding(
                    ...
                    (
                        ...
                        if data["data"]["next_funding_time"]
                        else None
                    ),
                    self.timestamp_normalize(int(data["data"]["funding_time"])),
                    predicted_rate=(
                        Decimal(data["data"]["estimated_rate"])
                        if data["data"]["estimated_rate"] is not None
                        else None
                    ),
                    raw=data,
                )
