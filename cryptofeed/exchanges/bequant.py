...
class Bequant(Feed):
    id = BEQUANT
    rest_endpoints = [
        RestEndpoint("https://api.bequant.io", routes=Routes("/api/2/public/symbol"))
    ]
            ...
            if pair in self.seq_no:
                if self.seq_no[pair] + 1 != msg["params"]["sequence"]:
                    if self.seq_no[pair] >= msg["params"]["sequence"]:
                        return
                    LOG.warning(
                        "%s: Missing sequence number detected for %s", self.id, pair
                    )
    ...
    async def authenticate(self, conn: AsyncConnection):
        if self.requires_authentication:
            ...
            signature = hmac.new(
                self.key_secret.encode("utf-8"), nonce, hashlib.sha256
            ).hexdigest()
    ...
    async def subscribe(self, conn: AsyncConnection):
        ...
        for chan, symbols in conn.subscription.items():
            ...
            if chan in [
                ...
                await conn.write(
                    json.dumps({"method": chan, "params": {}, "id": conn.uuid})
                )
            else:
                for symbol in symbols:
                    ...
                    await conn.write(
                        json.dumps({"method": chan, "params": params, "id": conn.uuid})
                    )
