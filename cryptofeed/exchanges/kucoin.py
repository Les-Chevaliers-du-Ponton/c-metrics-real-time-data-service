...
class KuCoin(Feed):
    ...
    rest_endpoints = [
        RestEndpoint(
            ...
            routes=Routes(
                "/api/v1/symbols", l2book="/api/v3/market/orderbook/level2?symbol={}"
            ),
        )
    ]
    ...
    def __init__(self, **kwargs):
        ...
        self.websocket_endpoints = [
            WebsocketEndpoint(
                address,
                options={
                    "ping_interval": address_info["data"]["instanceServers"][0][
                        "pingInterval"
                    ]
                    / 2000
                },
    ...
    async def _snapshot(self, symbol: str):
        ...
        data = await self.http_conn.read(
            self.rest_endpoints[0].route("l2book", self.sandbox).format(symbol),
            header=headers,
        )
        timestamp = time.time()
        ...
        self._l2_book[symbol] = OrderBook(
            self.id, symbol, max_depth=self.max_depth, bids=bids, asks=asks
        )
