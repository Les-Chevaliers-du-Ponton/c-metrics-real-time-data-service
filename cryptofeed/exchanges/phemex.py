...
class Phemex(Feed):
    ...
    websocket_endpoints = [
        WebsocketEndpoint(
            "wss://phemex.com/ws", sandbox="wss://testnet.phemex.com/ws", limit=20
        )
    ]
    rest_endpoints = [
        RestEndpoint(
            "https://api.phemex.com", routes=Routes("/exchange/public/cfg/v2/products")
        )
    ]
    price_scale = {}
    ...
    def _parse_symbol_data(cls, data: dict) -> Tuple[Dict, Dict]:
        ...
        for entry in data["data"]["products"]:
            ...
            info["tick_size"][s.normalized] = (
                entry["tickSize"] if "tickSize" in entry else entry["quoteTickSize"]
            )
    ...
    async def _book(self, msg: dict, timestamp: float):
        ...
        if msg["type"] == "snapshot":
            ...
            self._l2_book[symbol] = OrderBook(
                ...
                bids={
                    Decimal(entry[0])
                    / Decimal(self.price_scale[symbol]): Decimal(entry[1])
        ...
        await self.book_callback(
            L2_BOOK, self._l2_book[symbol], timestamp, timestamp=ts, delta=delta
        )
    ...
    async def message_handler(self, msg: str, conn: AsyncConnection, timestamp: float):
        ...
        if "id" in msg and msg["id"] == 100:
            if not msg["error"]:
                LOG.info(
                    "%s: Auth request result: %s", conn.uuid, msg["result"]["status"]
                )
                msg = json.dumps(
                    {
                        "id": 101,
                        "method": self.std_channel_to_exchange(BALANCES),
                        "params": [],
                    }
                )
            ...
            if not msg["error"]:
                LOG.info(
                    "%s: Subscribe to auth channels request result: %s",
                    conn.uuid,
                    msg["result"]["status"],
                )
            else:
