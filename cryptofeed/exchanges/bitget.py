...
class Bitget(Feed):
    ...
    websocket_endpoints = [
        WebsocketEndpoint(
            "wss://ws.bitget.com/spot/v1/stream", instrument_filter=("TYPE", (SPOT,))
        ),
        WebsocketEndpoint(
    ...
    async def _book(self, msg: dict, timestamp: float, symbol: str):
        ...
        if msg["action"] == "snapshot":
            ...
            if self.checksum_validation and self._l2_book[symbol].book.checksum() != (
    ...
    async def _login(self, conn: AsyncConnection):
        ...
        msg = hmac.new(
            bytes(self.key_secret, encoding="utf8"),
