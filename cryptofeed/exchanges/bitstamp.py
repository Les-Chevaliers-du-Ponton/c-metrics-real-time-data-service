...
class Bitstamp(Feed, BitstampRestMixin):
    ...
    websocket_endpoints = [
        WebsocketEndpoint("wss://ws.bitstamp.net/", options={"compression": None})
    ]
    rest_endpoints = [
        RestEndpoint(
            "https://www.bitstamp.net",
            routes=Routes(
                "/api/v2/trading-pairs-info/", l2book="/api/v2/order_book/{}"
            ),
        )
    ]
    websocket_channels = {
    ...
    async def _snapshot(self, pairs: list, conn: AsyncConnection):
        ...
        urls = [
            self.rest_endpoints[0].route("l2book", self.sandbox).format(sym)
            for sym in pairs
        ]
        ...
        for r, pair in zip(results, pairs):
            ...
            self._l2_book[std_pair] = OrderBook(
                ...
                asks={Decimal(u[0]): Decimal(u[1]) for u in r["asks"]},
                bids={Decimal(u[0]): Decimal(u[1]) for u in r["bids"]},
            )
            await self.book_callback(
                L2_BOOK,
