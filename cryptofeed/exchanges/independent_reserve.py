...
class IndependentReserve(Feed):
    ...
    async def _book(self, msg: dict, timestamp: float):
        ...
        for symbol in self.subscription[self.std_channel_to_exchange(L3_BOOK)]:
            if symbol.startswith(base):
                ...
                if (
                    instrument in self._sequence_no
                    and self._sequence_no[instrument] + 1 != seq_no
                ):
                ...
                if msg["Event"] == "OrderCanceled":
                    ...
                    raise ValueError(
                        "%s: Invalid OrderBook event message of type %s", self.id, msg
                    )
    ...
    async def _snapshot(self, base: str, quote: str):
        ...
        self._l3_book[normalized] = OrderBook(
            self.id, normalized, max_depth=self.max_depth
        )
        ...
        for chan, symbols in conn.subscription.items():
            if self.exchange_channel_to_std(chan) == L3_BOOK:
                subs.extend(
                    [
                        chan.format(s)
                        for s in set([sym.split("-")[0] for sym in symbols])
                    ]
                )
            else:
