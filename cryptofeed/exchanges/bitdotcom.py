...
class BitDotCom(Feed):
    ...
    websocket_endpoints = [
        ...
        WebsocketEndpoint(
            "wss://ws.bit.com",
            instrument_filter=("TYPE", (FUTURES, OPTION, PERPETUAL)),
            sandbox="wss://betaws.bitexch.dev",
        ),
    ]
    rest_endpoints = [
        RestEndpoint(
            "https://spot-api.bit.com",
            instrument_filter=("TYPE", (SPOT,)),
            sandbox="https://betaspot-api.bitexch.dev",
            routes=Routes("/spot/v1/instruments", authentication="/spot/v1/ws/auth"),
        ),
        RestEndpoint(
            "https://api.bit.com",
            ...
            routes=Routes(
                "/linear/v1/instruments?currency={}&active=true",
                currencies=True,
    ...
    def _symbol_endpoint_prepare(cls, ep: RestEndpoint) -> Union[List[str], str]:
        if ep.routes.currencies:
            return [
                ep.route("instruments").format(currency) for currency in ("USD", "USDT")
            ]
    ...
    def _parse_symbol_data(cls, data: list) -> Tuple[Dict, Dict]:
        ...
        for entry in data:
            if entry["code"] != 0:
                raise ValueError(
                    "%s - Failed to collect instrument data - %s",
                    cls.id,
                    entry["message"],
                )
            ...
            for mapping in entry["data"]:
                if "category" in mapping:
                    ...
                    # Spot
                    s = Symbol(
                        mapping["base_currency"], mapping["quote_currency"], type=SPOT
                    )
            ...
            for ep in self.rest_endpoints:
                if sym.type in ep.instrument_filter[1]:
                    ...
                    signature = self.get_signature(
                        ep.routes.authentication, {"timestamp": ts}
                    )
