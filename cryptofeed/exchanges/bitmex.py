...
class Bitmex(Feed, BitmexRestMixin):
    ...
    websocket_endpoints = [
        WebsocketEndpoint(
            ...
            sandbox="wss://testnet.bitmex.com/realtime",
    ...
    def init_order_info(self, o):
        oi = OrderInfo(
            ...
            (
                LIMIT
                if o["ordType"].lower() == "limit"
                else MARKET if o["ordType"].lower() == "market" else None
            ),
            ...
            raw=str(
                o
            ),  # Need to convert to string to avoid json serialization error when updating order
        )
        return oi
    ...
    async def _book(self, msg: dict, timestamp: float):
        ...
        self._l2_book[pair].timestamp = (
            ...
            if "data" in msg
            and isinstance(msg["data"], list)
            and msg["data"]
            and "timestamp" in msg["data"][0]
            else None
        )

        await self.book_callback(
            L2_BOOK, self._l2_book[pair], timestamp, raw=msg, delta=delta
        )
        ...
        for data in msg["data"]:
            ...
            f = Funding(
                ...
                self.timestamp_normalize(
                    data["timestamp"] + timedelta(hours=interval.hour)
                ),
                ts,
                raw=data,
            )
    ...
    async def message_handler(self, msg: str, conn, timestamp: float):
        ...
        if "table" in msg:
            if msg["table"] == "trade":
                ...
                LOG.warning(
                    "%s: Unhandled table=%r in %r", conn.uuid, msg["table"], msg
                )
        elif "info" in msg:
    ...
    async def _authenticate(self, conn: AsyncConnection):
        ...
        if self.key_id and self.key_secret:
            ...
            signature = hmac.new(
                self.key_secret.encode("utf-8"), msg, digestmod=hashlib.sha256
            ).hexdigest()
            await conn.write(
                json.dumps(
                    {"op": "authKeyExpires", "args": [self.key_id, expires, signature]}
                )
            )
