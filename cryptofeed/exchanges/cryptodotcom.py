...
class CryptoDotCom(Feed):
    ...
    rest_endpoints = [
        RestEndpoint(
            "https://api.crypto.com", routes=Routes("/v2/public/get-instruments")
        )
    ]
    ...
    async def _ticker(self, msg: dict, timestamp: float):
        ...
        for entry in msg["data"]:
            await self.callback(
                ...
                Ticker(
                    self.id,
                    self.exchange_symbol_to_std_symbol(entry["i"]),
                    entry["b"],
                    entry["k"],
                    self.timestamp_normalize(entry["t"]),
                    raw=entry,
    ...
    async def _book(self, msg: dict, timestamp: float):
        ...
        for entry in msg["data"]:
            ...
            self._l2_book[pair].book.bids = {
                price: amount for price, amount, _ in entry["bids"]
            }
            self._l2_book[pair].book.asks = {
                price: amount for price, amount, _ in entry["asks"]
            }
            ...
            await self.book_callback(
                ...
                self._l2_book[pair],
                timestamp,
                timestamp=self.timestamp_normalize(entry["t"]),
                raw=entry,
            )
