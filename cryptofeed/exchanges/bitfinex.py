...
class Bitfinex(Feed, BitfinexRestMixin):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super().__init__(
            symbols=symbols, channels=channels, subscription=subscription, **kwargs
        )
        ...
        if isinstance(msg, list):
            ...
            if chan_handler is None:
                if msg[1] == "hb":
                    ...
                    LOG.warning(
                        "%s: Unregistered channel ID in message %s", conn.uuid, msg
                    )
                    return
            seq_no = msg[-1]
            ...
            LOG.warning(
                "%s: Unexpected msg (missing event) from exchange: %s", conn.uuid, msg
            )
    ...
    def register_channel_handler(self, msg: dict, conn: AsyncConnection):
        ...
        LOG.debug(
            "%s: Register channel=%s pair=%s funding=%s %s -> %s()",
            conn.uuid,
            msg["channel"],
            pair,
            is_funding,
            "=".join(list(msg.items())[-1]),
            handler.__name__ if hasattr(handler, "__name__") else handler.func.__name__,
        )
    ...
    async def subscribe(self, connection: AsyncConnection):
        ...
        for chan, pairs in connection.subscription.items():
            for pair in pairs:
                message = {"event": "subscribe", "channel": chan, "symbol": pair}
                if "book" in chan:
                    parts = chan.split("-")
                    if len(parts) != 1:
