...
async def _playback(feed: str, filenames: list, callbacks: dict, config: str):
    ...
    if not callbacks:
        callbacks = {
            ctype: functools.partial(internal_cb, cb_type=ctype) for ctype in sub.keys()
        }
    else:
        for ctype in callbacks.keys():
            callbacks[ctype] = [
                callbacks[ctype],
                functools.partial(internal_cb, cb_type=ctype),
            ]
    feed = EXCHANGE_MAP[feed](
        candle_closed_only=False, config=config, subscription=sub, callbacks=callbacks
    )
        ...
        if endpoint:
            if header:
                self.data[uuid].append(
                    f"{endpoint} -> {timestamp}: {data} header: {json.dumps(header)}"
                )
            else:
                data = data.replace("\n", "")
