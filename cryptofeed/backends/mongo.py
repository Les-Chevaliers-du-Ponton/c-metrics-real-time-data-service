            ...
            async with self.read_queue() as updates:
                for index in range(len(updates)):
                    updates[index]["timestamp"] = (
                        dt.fromtimestamp(updates[index]["timestamp"], tz=timezone.utc)
                        if updates[index]["timestamp"]
                        else None
                    )
                    updates[index]["receipt_timestamp"] = (
                        dt.fromtimestamp(
                            updates[index]["receipt_timestamp"], tz=timezone.utc
                        )
                        if updates[index]["receipt_timestamp"]
                        else None
                    )
                    ...
                    if "book" in updates[index]:
                        updates[index] = {
                            ...
                            "bid": bson.BSON.encode(
                                updates[index]["book"]["bid"]
                                if "delta" not in updates[index]
                                else updates[index]["delta"]["bid"]
                            ),
                            "ask": bson.BSON.encode(
                                updates[index]["book"]["ask"]
                                if "delta" not in updates[index]
                                else updates[index]["delta"]["ask"]
                            ),
                        }
