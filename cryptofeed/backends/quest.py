...
class QuestCallback(SocketCallback):
    ...
    async def write(self, data):
        ...
        timestamp_int = (
            ...
            else received_timestamp_int * 1000
        )
...
class BookQuest(QuestCallback):
    ...
    async def __call__(self, book, receipt_timestamp: float):
        vals = ",".join(
            [
                f"bid_{i}_price={book.book.bids.index(i)[0]},bid_{i}_size={book.book.bids.index(i)[1]}"
                for i in range(self.depth)
            ]
            + [
                f"ask_{i}_price={book.book.asks.index(i)[0]},ask_{i}_size={book.book.asks.index(i)[1]}"
                for i in range(self.depth)
            ]
        )
        ...
        timestamp_int = (
            int(timestamp * 1_000_000_000)
            if timestamp is not None
            else receipt_timestamp_int * 1000
        )
...
class CandlesQuest(QuestCallback, BackendCallback):
    ...
    async def write(self, data):
        timestamp = data["timestamp"]
        timestamp_str = (
            f",timestamp={int(timestamp * 1_000_000_000)}i"
            if timestamp is not None
            else ""
        )
