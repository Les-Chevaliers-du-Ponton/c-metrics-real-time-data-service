...
class NBBO(Callback):
    ...
    def _update(self, book):
        ...
        min_ask = min(
            self.asks[book.symbol], key=lambda x: self.asks[book.symbol][x]["price"]
        )
        max_bid = max(
            self.bids[book.symbol], key=lambda x: self.bids[book.symbol][x]["price"]
        )

        return (
            self.bids[book.symbol][max_bid],
            self.asks[book.symbol][min_ask],
            max_bid,
            min_ask,
        )
